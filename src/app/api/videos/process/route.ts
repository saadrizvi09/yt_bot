...
import { extractVideoId, getVideoDetails, downloadAudio, transcribeWithAssemblyAI, chunkTranscript, getAutoGeneratedTranscriptWithYtDlp } from '@/lib/youtube';
  ...
  try {
    ...
    video = await db.video.create({
      data: {
        ...
        userId,
        // transcript: null, // No longer storing full transcript in Video model
      },
    });

    console.log('Video record created:', video.id);

    // Get transcript (prioritize yt-dlp auto-generated captions)
    console.log('Attempting to get auto-generated transcript with yt-dlp...');
    let transcript = await getAutoGeneratedTranscriptWithYtDlp(videoId);

    if (transcript) {
      console.log('Auto-generated transcript found from yt-dlp, length:', transcript.length);
    } else {
      console.log('No auto-generated transcript found from yt-dlp, falling back to AssemblyAI...');
      try {
    ...
    // No longer updating video with full transcript in Video model
    // console.log('Updating video with transcript...');
